...
#pragma pack(push, 1)
class Quest : public MonoBehaviour
{
	static Quest Instance; // 0x0
public:
	unsigned char x9[0x9];

	MessageProvider NameMessageProvider; // 0x20
	MessageProvider ShortDescriptionMessageProvider; // 0x28
	MessageProvider DescriptionMessageProvider; // 0x30
	MessageProvider RewardMessageProvider; // 0x38
	QuestEnums::QuestType Type; // 0x40
	bool SilentUpdate; // 0x44
	bool HideObjectiveMarker; // 0x45
	unsigned char x47[0x2];
	Vector2 Position; // 0x48
	unsigned char Icon[0x8]; // 0x50 Texture2D
	Quest* ChainQuest; // 0x58
	...
	Quest get_LastInChain()
	{
		auto get_LastInChain_address = Assembly_BaseAddr + 0xB223F0;
		using __get_LastInChain = void(*)(Quest*);
