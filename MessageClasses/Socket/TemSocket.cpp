...
void TemSocket::ListenLoop(LPVOID lpParameter)
{
	...
	{
		isBytesLeft = ReadyToReceive(static_cast<int>(ListenSocket), 0);// recv(ListenSocket, recvbuf, recvbuflen, MSG_PEEK);
		...
		if (MessageIsComplete == true && ReceivedMessage.size() > 0)
		{
			...
			case sutil::hash("1"):
			{
				bytesSent = send(ListenSocket, sendbuf, static_cast<int>(strlen(sendbuf)), 0);
			...
			case sutil::hash("GETGHOSTS"):
			{
				if (message.size() > 2)
				{
					...
					for (std::size_t i = 2; i < message.size(); i+= 2)
					{
...
void TemSocket::ProcessMessageQueue()
{
	while (MessageQueue.size() > 0 && TemSocket::IsConnected == true)
	{
		...
		while (position < message.size() && TemSocket::IsConnected == true)
		{
			...
			bytesSent = send(ListenSocket, sendbuf, std::min<int>(static_cast<int>(message.size() - position), 4096), 0);
